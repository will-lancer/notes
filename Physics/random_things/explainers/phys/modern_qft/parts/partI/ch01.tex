...
diagrams with the usual symmetry factors. So far, so textbook. But apply
...
embarrassing in their simplicity. Their force comes from dimensional promotion:
...
constructive literature is reviewed in Dedushenko \arxiv{2203.08053}, \S2.1,
which is also the source for the axiom systems quoted later in this chapter.}
...
\noin Four cracks, one diagnosis. In each case the failure is not of quantum
...
Osterwalder--Schrader, Haag--Kastler, and Atiyah--Segal axiom systems; see
Dedushenko \arxiv{2203.08053}, \S2, for all of them with full references, and
Section~\ref{sec:answer} below for how they compare. The geometric (``assign
...
reverse. Keep this little duality in mind as a template: \emph{the same
observable, two expansions, each weakly coupled where the other is not}. That
sentence, suitably inflated, is Part VI.
\end{eexample}
...
\vocab{universality} in its smallest habitat, and it is the engine that makes
...
The final clause earns its keep: the failure modes are sharp and instructive. A
...
what data survives the trip --- is a running concern of these notes, climaxing
in Part VIII.
...
(Part VIII.) The picture is simple; its load-bearing capacity is the discovery
...
survey behind Section~\ref{sec:answer}: every axiom system, with its successes
and its stress fractures, in 30 readable pages plus a heroic bibliography. Read
\S\S1, 3 now and \S2 as reference.
