...
% --- local macros for the annotated TOC ---
...
\noin
An annotated table of contents for the quantum field theory textbook of $\sim$2046: the book that will exist once the developments of 2004--2026 --- generalized and non-invertible symmetries, the bootstrap renaissance, monotonicity theorems, the algebraic/entanglement revival, the lattice--continuum program --- are digested into pedagogy the way Wilson's revolution was digested into the textbooks of the 1990s. Each chapter comes with a paragraph explaining why it exists and pointers into the current literature (Seiberg, Witten, Shao, C\'ordova, Sch\"afer-Nameki, Rychkov, Simmons-Duffin, Komargodski, Gaiotto, Rastelli, Yin, Polyakov's heirs all). The cited papers are collected in \texttt{assets/}, indexed by arXiv number; see \texttt{assets/README.md}.
...
Parts I--IV are the spine: the arena (I), the exact structures on it (II), its landmarks (III), and its dynamics (IV). Parts V--VIII are dynamics in the flesh: gauge theories (V), the exactly solvable corners that anchor everything (VI), duality (VII), and the lattice--continuum dictionary where the definition of the subject is stress-tested (VIII). Parts IX--XI are interfaces with quantum information and gravity. Part XII is the frontier. A reader of 2026 wanting one path: I $\to$ II $\to$ III.1--III.2 $\to$ IV.1 $\to$ V $\to$ VIII, with VI and IX read in parallel.

\noin Three one-semester courses are embedded in the book: \emph{symmetry-first QFT} (Parts I, II, V, VIII --- the Seiberg school); \emph{the bootstrap} (Parts I, III, IV, X --- the Polyakov program executed); \emph{fields, information, and gravity} (Parts I, IX, XI). Three lighthouse theories recur in every part and should be solved completely, once, early: the 2d Ising model with its Majorana chain (symmetry, duality, the lattice), the 3d Ising CFT (bootstrap, defects, fuzzy sphere), and Maxwell/Yang--Mills in 4d (higher-form symmetry, $\theta$, lines). When these notes graduate from table of contents to text, the writing should start where today's textbooks are thinnest: Parts II and VIII exist only as lecture notes and research papers.

\newpage

% =====================================================================
\section{The Arena: Quantum Field Theory Without Lagrangians}

\noin \emph{What is being defined before any method of computing it: observables, states, and the space of theories.}

\subsection{What is a quantum field theory?}
...
\subsection{Observables I: local operators}
...
\chreading{Rychkov \arxiv{1601.05000} (chs.\ 1--2); Kontsevich--Segal \arxiv{2105.10161} (\S2, for which Euclidean backgrounds are allowed).}

\subsection{Observables II: defects and extended operators}
...
\subsection{States, geometry, and cutting-and-gluing}
...
\subsection{Presentations, not definitions}
...
\subsection{Theory space and the renormalization group}
...
\chreading{Rychkov \arxiv{1601.05000} (ch.\ 1); Casini--Huerta \arxiv{2201.13310} (\S1 for the entropic restatement, used in Part IX).}
...
\subsection{Symmetries are topological operators}
...
\chreading{Gaiotto--Kapustin--Seiberg--Willett \arxiv{1412.5148} (\S1--2); Shao \arxiv{2308.00747} (ch.\ 1); Iqbal \arxiv{2407.20815} (gentlest entry).}

\subsection{Gauging and 't Hooft anomalies}
Gauging as summing over backgrounds; 't~Hooft anomalies as the obstructions to doing so, and anomaly \emph{matching} as the resulting exact RG invariant --- the sharpest nonperturbative tool in the subject. Anomaly inflow: anomalies of a $d$-dimensional theory are classified by invertible/SPT phases in $d+1$ dimensions. Perturbative (chiral) and global (parity, time-reversal, discrete) anomalies in one framework.
...
\chreading{Shao \arxiv{2308.00747}; Sch\"afer-Nameki \arxiv{2305.18296}; C\'ordova--Dumitrescu--Intriligator--Shao \arxiv{2205.09545} (panorama with full bibliography).}
\chfurther{Witten--Yonekura (\texttt{1909.08775}) for $\eta$-invariants and the deepest version of inflow.}

\subsection{Higher-form symmetries and the global structure of gauge theories}
...
\subsection{Non-invertible symmetries}
...
\subsection{The Symmetry Topological Field Theory}
...
\subsection{Anomalies in parameter space}
...
\subsection{The generalized Landau paradigm}
...
% =====================================================================
\section{Fixed Points: Conformal Field Theory}

\noin \emph{The landmarks of theory space, studied as theories-in-themselves with no microscopic crutch. Polyakov's 1974 program, finally executed.}

\subsection{Conformal kinematics in $d \ge 3$}
The conformal algebra and its unitary representations; primaries and descendants; radial quantization and the state--operator map (the cleanest argument for the OPE's convergence); unitarity bounds; correlators fixed by symmetry up to functions of cross-ratios; embedding space.
\chsections{conformal algebra and representations; radial quantization; unitarity bounds; two- and three-point functions; conformal blocks; the convergent OPE.}
\chreading{Rychkov \arxiv{1601.05000}; Simmons-Duffin \arxiv{1602.07982} (the standard modern courses).}

\subsection{The conformal bootstrap}
...
\chfurther{El-Showk et al.\ (\texttt{1203.6064}), the paper that started the modern era.}

\subsection{The analytic bootstrap}
...
\chfurther{Caron-Huot--Maz\'a\v{c}--Rastelli--Simmons-Duffin (\texttt{2008.04931}) for dispersive sum rules.}
...
\subsection{Boundaries, defects, and interfaces in CFT}
Conformal defects as CFTs' richest substructure: BCFT and the Cardy condition; defect operator expansions and bulk-to-defect data; line defects as quantum mechanics coupled to a CFT (the Wilson line as a renormalization problem); the defect bootstrap; defect RG flows and the $g$-theorem (proved entropically in Part IX).
\chsections{BCFT; Cardy states; defect CFT data and crossing; magnetic lines in the 3d Ising model; defect RG.}
\chreading{Northe \arxiv{2411.03381}; Billò--Gon\c{c}alves--Lauria--Meineri (\texttt{1601.02883}).}

\subsection{New ways to solve a CFT}
...
\chfurther{Hellerman et al.\ (\texttt{1505.01537}), the large-charge original.}

% =====================================================================
\section{Flows: The Renormalization Group as Dynamics}

\noin \emph{What can be said exactly about the flow between landmarks: monotonicity, matching, and the strange solvable deformations.}
...
\chsections{$c$; $a$ via dilatons; $F$ and sphere partition functions; entropic unification (preview of Part IX); defect monotonicity; uses: ruling out IR scenarios.}
\chreading{Komargodski--Schwimmer \arxiv{1107.3987}; Casini--Huerta \arxiv{2201.13310}; Klebanov--Pufu--Safdi (\texttt{1105.4598}).}

\subsection{Constraining the deep infrared}
...
\subsection{The edge of criticality}
...
\chreading{Rychkov--Su \arxiv{2311.15844} (\S on conformal windows); Gorbenko--Rychkov--Zan (complex CFTs; see references in the above).}

\subsection{Solvable deformations and the integrable corner}
...
\chreading{Smirnov--Zamolodchikov (\texttt{1608.05499}); Jiang \arxiv{1904.13376} (pedagogical).}

% =====================================================================
\section{Gauge Dynamics}
...
\subsection{What is a gauge theory?}
...
\chsections{$\theta$-dependence generalities; the $\theta=\pi$ anomaly; walls and interfaces; large $N$; axions and Part II.6 revisited.}
\chreading{Gaiotto--Kapustin--Komargodski--Seiberg \arxiv{1703.00501}; C\'ordova--Freed--Lam--Seiberg \arxiv{1905.09315}.}

\subsection{Low-dimensional laboratories}
...
\subsection{The Standard Model through modern eyes}
...
% =====================================================================
\section{Exactly Solvable Corners: Supersymmetry and Friends}
...
\subsection{Holomorphy and Seiberg duality}
$\mathcal{N}=1$ theories as the minimal setting where strong-coupling questions have exact answers: holomorphy as the supersymmetric replacement for perturbation theory; moduli spaces of vacua; Seiberg duality as the prototype of IR duality --- and the historical seed of the entire duality-centric worldview.
...
\chreading{Tachikawa \arxiv{1312.2684} (companion volume); Intriligator--Seiberg lectures (classic, see references therein).}
...
\subsection{Localization and exact partition functions}
...
\chreading{Pestun (\texttt{0712.2824}); the review volume \emph{Localization Techniques in QFT} (\texttt{1608.02952}).}

\subsection{Theories without Lagrangians}
...
\subsection{Protected subsectors and integrability}
Exact substructures inside non-integrable theories: the chiral algebra (2d VOA) inside every 4d $\mathcal{N}=2$ SCFT --- Rastelli school --- feeding exact CFT data into the bootstrap; topological sectors; integrability of planar $\mathcal{N}=4$ as the maximal solvable corner; 2d factorized S-matrices (used again in Part X).
...
\subsection{The 2+1d duality web}
...
\chreading{Seiberg--Senthil--Wang--Witten \arxiv{1606.01989}; Senthil--Son--Wang--Xu (review; see references therein).}

\subsection{Duality from gauging}
The mechanism beneath the webs: dualities generated by gauging discrete symmetries (orbifolding), with Kramers--Wannier as the original example; self-duality producing non-invertible symmetry (Part II.4 closed into a loop); the SymTFT as the bookkeeping that makes whole duality webs one picture.
...
% =====================================================================
\section{The Lattice and the Continuum}
...
\subsection{Continuum limits, reexamined}
...
\subsection{Lieb--Schultz--Mattis theorems as anomalies}
...
\subsection{Exotica: fractons and UV/IR mixing}
...
\subsection{Rigor: constructive QFT and triviality}
...
\subsection{Information-theoretic theorems}
...
\subsection{Coupling to gravity: the crossed product}
...
% =====================================================================
\section{Scattering, the UV, and the Space of Consistent Theories}
...
\subsection{The S-matrix bootstrap}
...
\subsection{Positivity and the UV--IR connection}
...
\chreading{Adams--Arkani-Hamed--Dubovsky--Nicolis--Rattazzi \arxiv{hep-th/0602178}; de Rham et al.\ (\texttt{2203.06805}); Arkani-Hamed--Huang--Huang (\texttt{2012.15849}).}

\subsection{Toward flat-space holography}
...
\chreading{Kruczenski--Penedones--van~Rees \arxiv{2203.02421} (\S outlook); Strominger's lectures (see references therein).}

% =====================================================================
\section{Large $N$, Holography, and Gravity}
...
\subsection{Large $N$ as a classical limit}
...
\subsection{Holography from the CFT side}
...
\chreading{Heemskerk--Penedones--Polchinski--Sully \arxiv{0907.0151}; Penedones \arxiv{1608.04948}.}
...
\subsection{Quantum field theory as quantum gravity}
The strongest claim in the book: in AdS, quantum gravity is \emph{defined} by a QFT, making everything in Parts I--IX a statement about quantum gravity. What gravity has paid back: the chaos bound, energy conditions, the algebraic reorganization of Part IX.4; black-hole information as a theorem about unitary QFT.
...
\chreading{Penedones \arxiv{1608.04948}; Maldacena--Shenker--Stanford (\texttt{1503.01409}).}

% =====================================================================
\section{Epilogue: The Next Twenty Years}

\noin \emph{What the 2046 book admits it does not know --- the chapters that will be rewritten by 2066.}

\subsection{The definitional frontier}
...
\subsection{Beyond quantum field theory}
...
\noin All papers below are in \texttt{assets/}, with filenames beginning with the arXiv number; \texttt{assets/README.md} maps them to the parts above. Lecture notes and reviews are marked $\bullet$; foundational research papers $\circ$.
...
\subsection*{Gauge dynamics and duality (Parts V, VII)}
\begin{itemize}
...
\subsection*{Exact corners (Part VI)}
\begin{itemize}
...
\subsection*{Quantum information (Part IX)}
\begin{itemize}
...
\subsection*{Scattering and the UV (Part X)}
\begin{itemize}
...
\subsection*{Methods (Parts III.6, IV.4, and beyond)}
\begin{itemize}
