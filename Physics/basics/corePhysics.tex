...
\vskip 16pt
{\fontsize{36}{36} \selectfont \bf \sffamily Core Physics}
\vskip 24pt
...
\noin
These are notes on basic physics, covering classical mechanics,
classical field theory, statistical mechanics, and quantum mechanics.
Resources used: \note{fill in later}
\begin{itemize}
    \item Classical mechanics:
    \item Classical field theory:
    \item Statisical mechanics:
...
\note{need to re-learn this subject; i'm excited to do that, though!
really looking forward to it. tong for a first pass, then goldbart,
then landau, then arnol'd? something like that}

\subsection{Phase space}

\subsubsection{Generalities}

Regardless of how we derive the Hamiltonian, we first
must define the canonical momentum $\bfp = \partial_{\dot{\bfq}} L$.
\note{comment on this---what's deep about the canonical momentum?
how can it be motivated? maybe in relation to the ele but eh, feels weak}
Then, we can define the \vocab{Hamiltonian} of a system as the Legendre 
transform of the Lagrangian for that system,
\begin{align*}
    H \coloneqq \bfp \cdot \dot{\bfq} - L|_{\bfp = \partial_{\dot{\bfq}} L}.
\end{align*}

\textcolor{red}{todos:
\begin{itemize}
    \item cover legendre transform in an idea box; put it before the expression
    for the lagrangian; use Arnol'd to do this
    \item motivate the canonical momentum; why is it useful? i don't really get it
    \item two-form picture of Hamiltonian mechanics
\end{itemize}}
...
\subsubsection{Symplectic Mechanics}
...
We'll be using index notation throughout these notes, so most vector identities
are useless to put here. We will simply record those identities that are hard to
remember.

\subsubsection{Differential identities}

\begin{align*}
    \grad^2_{\rm sph} & = \frac{1}{r^2} \partial_r (r^2 \partial_r) + \frac{1}{r^2} \underbrace{\left( \frac{1}{\sin \theta} \partial_\theta (\sin \theta \partial_\theta) + \frac{1}{\sin^2 \theta} \partial_\phi^2 \right)}_{\grad^2_{\Omega}}\\
    \grad^2_{\rm cyl} & = \frac{1}{r} \partial_r (r \partial_r) + \frac{1}{r^2} \partial_\phi^2 + \partial_z^2\\
\end{align*}

\subsubsection{Integral identities}
...
\begin{align*}
    S[A] = \int d^4 x \, \left( - \frac{1}{4} F_{\mu \nu} F^{\mu \nu} + j^\mu A_\mu \right),
\end{align*}
where $F_{\mu \nu} \coloneqq 2 \partial_{[\mu} A_{\nu]}$, and $j^\mu$ our current.
Varying this action gives the equations of motion
\begin{align*}
    \fd{S[A]}{A_\nu} = 0 \implies \boxed{\partial_\mu F^{\mu \nu} = - j^\nu.}
\end{align*}
There are also two natural constraints that arise from this construction:
the first comes from the anti-symmetry of $F$,
\begin{align*}
    \partial_\nu \partial_\mu F^{\mu \nu} = - \partial_\nu j^\nu = 0 \implies \boxed{\partial_\mu j^\mu = 0.}
\end{align*}
Thus, to couple the matter sector of electromagnetism with our $U(1)$ gauge
field, the current must be locally conserved; you can also view this as a consequence
of demanding that $S[A]$ is gauge invariant\footnote{You can trick yourself into thinking
that the conservation of $j^\mu$ arises due to some sort of ``Noether's theorem'' applied to 
gauge redunancy---this is, of course, false. The conservation of current in electromagnetism
is a \emph{consistency condition} for matter to couple with electromagnetism, not a conservation
law in the sense of Noether.} Additionally, from the definition of $F_{\mu \nu}$, we have that
\begin{align*}
    \boxed{\epsilon^{\mu \nu \rho \sigma} \partial_\nu F_{\rho \sigma} = 0.}
\end{align*}
One can see this is true as follows: write out $\partial_\nu F_{\rho \sigma} = \partial_\nu \partial_\rho A_\sigma - \partial_\nu \partial_\sigma A_\rho$
and then contract with $\epsilon$, renaming indices in the second term:
\begin{align*}
    \epsilon^{\mu \nu \rho \sigma} \partial_\nu F_{\rho \sigma}
    & = \epsilon^{\mu \nu \rho \sigma} (\partial_\nu \partial_\rho A_\sigma - \partial_\nu \partial_\sigma A_\rho)\\
    & = \epsilon^{\mu \nu \rho \sigma} \partial_\nu \partial_\rho A_\sigma - \epsilon^{\mu \nu \sigma \rho} \partial_\nu \partial_\rho A_\sigma\\
    & = \epsilon^{\mu \nu \rho \sigma} \partial_\nu \partial_\rho A_\sigma + \epsilon^{\mu \nu \rho \sigma} \partial_\nu \partial_\rho A_\sigma\\
    & = 2 \epsilon^{\mu \nu \rho \sigma} \partial_\nu \partial_\rho A_\sigma = 2 \epsilon^{\mu [\nu \rho] \sigma} \partial_{(\nu} \partial_{\rho)} A_\sigma = 0.
\end{align*}
Together, these two equations constitute \vocab{Maxwell's equations}:
\begin{align*}
    & \boxed{\partial_\mu F^{\mu \nu} = - j^\nu,}
    & \boxed{\epsilon^{\mu \nu \rho \sigma} \partial_\nu F_{\rho \sigma} = 0.}
\end{align*}
Besides the equations of motion, we'd like to consider the symmetries of classical
electromagnetism. Generically, classical electromagnetism has gauge redunancy and
translational symmetry, with the latter giving rise to the stress-energy-momentum
tensor:
\begin{align*}
    j^\mu = \pd{\el}{(\partial_\mu A_\nu)} \delta A_\nu - \mathcal{U}^\mu = - F^{\mu \nu} a^\rho \partial_\rho A_\nu - a^\rho \delta^\mu_{\rho} \el = - a^\rho T^\mu{}_\rho,
\end{align*}
where
\begin{align*}
    T^\mu{}_\nu = F^{\mu \rho} \partial_\nu A_\rho + \delta^{\mu}_\nu \el
...
\subsection{Conductor electrostatics}
...
is just a way of rewriting Gauss' law by taking the curl-free
...
\begin{reemark}
    ...
    right. While we may always define a local $\Phi$ such that $E = - d \Phi$,
    a global $\Phi$ satisfying such a condition requires Poincar\'e's lemma
    to hold, and thus our domain must be simply connected.
\end{reemark}

\subsubsection{Field properties}

In addition to the equations of motion, we should naturally consider
the symmetries---and thus conserved currents---of electromagnetism.
Recall the improved stress-energy-momentum tensor for electromagnetism:

\begin{align*}
    T^\mu{}_\nu = \note{put here}
\end{align*}

\note{need to learn more about this before writing this section}

\textcolor{red}{
    \begin{itemize}
        \item read Griffiths ch. 7
        \item landau...
        \item tong...
    \end{itemize}
}

\subsubsection{Inhomogeneous solutions}

We now treat potential problems. The only way we solve Poisson's equation in 
...
\begin{iidea}
    ...
    to a $\delta$-function source; heuristically, it's a weighting factor for a bunch
    of $\delta$-function sources that each contribute to your solution $\psi$. The
    physical analogy here is $\delta$-functions $\leftrightarrow$ point charges,
    so $G$ basically just counts a bunch of tiny point charges that all contribute
    to the ``potential'', $\psi$.
\end{iidea}

Some comments on Green's functions, both in generality
and in the context of electrostatics:
\begin{itemize}
    \item Green's functions are, \emph{a priori}, not unique. This is because
    we may add homogeneous solutions to $G$ and still satisfy (\ref{greensFunc}).
    This indeterminacy is solved by implementing boundary conditions on $G$. We should
    expect $G$ to be ill-defined before implementing boundary conditions,
    as we're still solving a PDE, and boundary conditions are necessary to specify 
    unique solutions to PDEs. Before we specify boundary conditions, solving (\ref{greensFunc})
    actually solves for an entire equivalence class of Green's functions
    for the operator $\el$. More concretely, if $\Lambda$ obeys $(\el \Lambda)(x, x') = 0$,
    then
    \begin{align*}
...
\note{do this derivation}
You can find this via Fourier transform. Integrating this 
result gives the potential for a localized charge distribution:
\begin{align}
...
expanding $G$ in the eigenfunctions of $-\grad^2$. \note{fix
all of the below after learning about eigenfunction expansions
more broadly}
...
We are physicists, so it's uncommon that we take more than the leading
term in any sort of approximation. Thus, it's useful to give names to the
first few terms in the more common expansions; in the Cartesian expansion,
we write
\begin{align*}
    & \mathbf{p} \coloneqq \int d^3 \bfx' \, \rho(\bfx') \, \bfx',
    & \mathbb{Q}_{ab} \coloneqq \int d^3 \bfx' \, \rho(\bfx') \left( \frac{3x_a' x_b' - (r')^2 \delta_{ab}}{r^5} \right).
\end{align*}
These are respectively called the electric \vocab{dipole} and \vocab{quadrupole}
moments. Whether a potential goes like its dipole or quadrupole moment can oftentimes
be determined by inspection: if there is reflection symmetry in a given direction,
you know that the dipole moment in that direction is zero. \note{wb for quadrupole?
give examples} Inserting the definitions of the dipole and quadrupole moments
into the expression (\ref{freeFieldPotential}) gives
\begin{align*}
    \Phi(\bfx) \simeq \frac{1}{4\pi\epsilon_0} \left( \frac{Q}{r} + \frac{\bfx \cdot \bfp}{r^3} + \frac{x_a x_b \mathbb{Q}^{ab}}{r^5} + \bigo((r')^3) \right)
\end{align*}
We can also define spherical moments: assume that $r' < r$. Then, we have
that
\begin{align*}
    q_{\ell m} \coloneqq \int d^3 \bfx' \, \rho(\bfx') (r')^\ell Y_{\ell m}^*(\Omega')
\end{align*}
Inserting this definition into (\ref{freeFieldPotential}) gives
\begin{align*}
    \Phi(\bfx) & = \frac{1}{4\pi\epsilon_0} \sum_{\ell = 0}^\infty \frac{4\pi}{2\ell + 1} \sum_{m = -\ell}^\ell \frac{q_{\ell m}}{r^{\ell + 1}}\\
    & \simeq \frac{1}{4\pi\epsilon_0} \left( 4\pi \frac{q_{00}}{r} + \frac{4\pi}{3} \frac{q_{1, -1} + q_{10} + q_{11}}{r^2} + \bigo((r')^3) \right).
\end{align*}
\note{write out the first few spherical moments}

\begin{eexample}
    \note{do an example or two here; talk about method of images as well}
\end{eexample}
...
We now consider \vocab{Laplace's equation}, the homogeneous counterpart
...
\note{need to get a lot more clear on what an eigenfunction expansion
is---i really don't know what it is}
...
\subsection{Dielectric electrostatics}
...
ME tells us how the $\magn$ field is sourced---notice the qualitative
difference between the sourcing of $\elec$ and $\magn$: $\elec$
is related to its source via a divergence, while $\magn$ is related
to its source via a curl. Combining these two equations and choosing 
the Coulomb gauge gives our Poisson equation, and the Lorentz force 
law tells us how the magnetic field couples to charged matter.
...
\note{derive the results for the dirichlet and neumann bc on vector potential}

\note{in approx schemes, talk about multi-pole expansion etc}
...
The fundamental idea behind statistical mechanics is that it is
mechanics, treated statistically. Statistical mechanics aims to make 
predictions about the bulk properties of systems with a large number of 
degrees of freedom. ``Large'' may generally be defined as
when the number of particles is on the order of Avogadro's number,
$N_A = 6.022 \times 10^{23}$ mol$^{-1}$, where the mol$^{-1}$ in the 
expression for $N_A$ comes from the fact that we want to write things 
like $N = n N_A$.

\begin{reemark}
    [Avogadro's number]
    Historically, Avogadro's number was defined as the number of atoms 
    in $12 \text{ g}$ of carbon-12. Here is a less stupid derivation of it: 
    just comparing length scales.

    An atom is on the order of $10^{-10} \, \unit{m}$, and a centimeter $10^{-2} \, \unit{m}$.
    Considering the number of atoms in $1 \, \unit{cm}^3$ gives
    \begin{align*}
        \# \text{ atoms} \sim \frac{10^{-6}}{10^{-30}} = 10^{24} \, \unit{atoms}
    \end{align*}
    which is about the same OOM as Avogadro's number. Another derivation
    comes through considering that the scale of atomic mass is $10^{-26} \, \unit{kg}$,
    while the gram is $10^{-3} \, \unit{kg}$, and thus there are roughly
    $10^{23}$ atoms per gram of mass.
\end{reemark}

While we may define thermodynamic quantities when
the number of particles is on the order of Avogadro's number, many results 
in thermodynamics only hold in the \vocab{thermodynamic limit}; this is when you 
send both the particle number and volume of the system to infinity, but keep the density
constant. This limit is actually quite deep---its effects will become most noticable 
when we talk about phase transitions later in these notes, as we need this limit
to make sense of even basic ideas in the subject.

\subsection{The microscopic picture}
...
The fundamental object in statistical mechanics is the \emph{density operator},
$\rho$. We recall its definition here. If there is a probability $p_i$ that
a system is in state $\ket{\psi_i}$, the density operator for that system
is given by 
\begin{align*}
    \rho = \sum_i p_i \ketbra{\psi_i}{\psi_i}.
\end{align*}
For a system in a pure state $\ket{\psi}$, clearly $\rho = \ketbra{\psi}{\psi}$.
It can be shown that a density operator generically satisfies $\tr{\rho} = 1$
and $\rho \geq 0$; this makes sense, as we'd like to think about $\rho$ as
a probability distribution of sorts. While $\rho$ is the fundamental object
of statistical mechanics---as the world is fundamentally quantum mechanical---treating
the microscopic dynamics as \emph{classical} is oftentimes an almost
perfect approximation to the quantum mechanical theory. This is, of course,
due to the renormalization group---different microscopic theories oftentimes
give way to the same effective theory. Thus, we are also interested
the classical Hamiltonian of our system, $H$.

The Hamiltonian of a system is a smooth function on phase space that gives the
dynamics of the system when differentiated. Concretely, if we have a system with 
$N$ degrees of freedom, the phase space is generically $2N$-dimensional, and the Hamiltonian
is a function $H = H(\bfp, \bfq)$ on this space. The state of the system
evolves due to \emph{Hamilton's equations},
\begin{align*}
    & \dot{\bfq} = \pd{H}{\bfp},
    & \dot{\bfp} = - \pd{H}{\bfq}.
\end{align*}
\note{talk about symplectic two form here, as well as Liouville's theorem}


Along with these fundamental objects, we must make a fundamental assumption
called the \vocab{ergodic hypothesis}.

\begin{iidea}
    [The Ergodic Hypothesis]
    The ergodic hypothesis states that over long times, a system explores
    all possible accessible microstates at an energy $E$. Thus, time averages 
    equal ensemble averages. \note{fix this ``definition'' after reading the
    paper}
\end{iidea}

\begin{reemark}
    [Is the ergodic hypothesis true?]
    Yes and no. For most systems, yes, the ergodic hypothesis seems to 
    hold just fine. But there are classes of systems where it does not hold.
    We can further delineate between these two classes by observing \emph{how}
    they break ergodicity. There are those systems whose time scales for exploration
    are just incredibly long (e.g. longer than the time to the end of the universe),
    and thus practically break the hypothesis: this is \vocab{weak breaking}.
    Then there are those systems who will \emph{never} explore all of the states
    available to them, regardless of the time they have to access the states:
    this is \vocab{strong breaking}.

    Examples of weak breaking may be found in spin glasses and generic finite
    systems, where the latter case is an example of the Poincare recurrence theorem.
    To recall from the notes on classical mechanics, the Poincare recurrence theorem
    roughly states that chaotic systems return arbitarily close to their initial
    state. This practically restricts the volume of phase space that the system explores,
    thus breaking ergodicity. Spin glasses are just complicated systems with lots of
    potential barriers, thus making it really difficult to explore microstates; the time
    scale for exploration becomes absurdly long as the system gets larger, thus weakly
    breaking ergodicity.

    The examples of strong breaking are much more interesting and deep. The quintessential
    example of strong breaking is an \emph{integrable} system. That is, a Hamiltonian
    system with as many conserved quantities as degrees of freedom. \note{expand}
\end{reemark}

\begin{eexample}
    \note{give concrete example of where it fails}
\end{eexample}

We now talk about the three different ``ensembles'' of thermodynamics.
We start with the microcanonical ensemble. A priori, the distinguishing
factor delineating the three ensembles from each other is the variables
that are assumed to be held constant for that ensemble. In the microcanonical
ensemble, energy, volume, and particle number are held constant.

\note{at some point talk about special relativistic thermodynamics}
...
\begin{align*}
    (E - H_0)^{-1} = \sum_{k, \, \alpha} \frac{\ketbra{k\alpha}{k\alpha}}{E - \epsilon_k}.
\end{align*}
