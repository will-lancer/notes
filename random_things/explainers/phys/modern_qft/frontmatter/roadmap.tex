...
Wilson's picture promoted to the book's geography. RG flow on the space of theories; fixed points, relevant/irrelevant/marginal directions as the tangent space; beta functions, Callan--Symanzik equations, and operator mixing as coordinate expressions of that geometry; universality as the statement that flows converge; effective field theory as local coordinates; conformal manifolds; lines of fixed points; the questions the rest of the book answers (what is exactly transported along flows? --- Part II; what are the fixed points? --- Part III; which flows are allowed? --- Part IV).
\chsections{Wilsonian RG without a cutoff fetish; scheme-independence and what is physical; beta functions and Callan--Symanzik; local RG and operator mixing; EFT; universality classes; conformal manifolds and exactly marginal deformations.}
...
Gauging as summing over backgrounds; 't~Hooft anomalies as the obstructions to doing so, and anomaly \emph{matching} as the resulting exact RG invariant --- the sharpest nonperturbative tool in the subject. Anomaly inflow: anomalies of a $d$-dimensional theory are classified by the invertible/SPT phases of chapter II.2 in $d+1$ dimensions. Perturbative chiral anomalies, global anomalies, parity and time-reversal anomalies, mixed gauge-gravitational anomalies, and the spin/Pin structures needed to state fermionic theories all appear in one framework.
\chsections{gauging finite and continuous symmetries; 't Hooft's argument; inflow and the bulk-boundary picture; fermion parity and spin/Pin backgrounds; SPT phases and the cobordism classification (sketch); discrete, global, and mixed gravitational anomalies; examples from QCD to the honeycomb lattice.}
...
\chfurther{Witten--Yonekura (\texttt{1909.08775}) for $\eta$-invariants and the deepest version of inflow; Delmastro--Gaiotto--Gomis (\texttt{2101.02218}) for global anomalies on Hilbert spaces and spin TQFT data.}
...
\roadchapter{IV.3}{Effective field theory of broken symmetries}
The most useful IR coordinates are often not fields in the UV Lagrangian but Goldstone modes and background responses. Goldstone's theorem and the coset construction; nonlinear sigma models; Wess--Zumino and WZW terms as anomaly matching written in an action; chiral perturbation theory and pions; phonons, superfluids, and spacetime-symmetry breaking; SMEFT as the symmetry-compatible deformation of the Standard Model. This is the bridge between the exact constraints of IV.2 and the practical EFTs used in Parts V and IX.
\chsections{Goldstone theorem; cosets and inverse Higgs; sigma models; WZ/WZW terms and anomalies; chiral perturbation theory; spacetime symmetry breaking; SMEFT.}
\chreading{Naegels (\texttt{2110.14504}) for Goldstone bosons and cosets; Weinberg, \emph{The Quantum Theory of Fields II}, for the durable EFT viewpoint.}

\roadchapter{IV.4}{The edge of criticality}
...
\roadchapter{IV.5}{Solvable deformations and the integrable corner}
...
\roadchapter{IV.6}{What perturbation theory means}
...
\roadchapter{IV.7}{Hydrodynamics: the effective theory of thermal states}
...
The SM as a case study in everything above: anomaly cancellation and what would fail without it; the global form of its gauge group and the line operators that distinguish the candidates; its accidental, higher-form, and non-invertible symmetries ($B-L$, the axial $U(1)$); pions and WZW terms from anomaly matching; neutrino masses, SMEFT, and naturalness questions restated symmetry-first.
\chsections{which SM? (global structures); anomaly cancellation and mixed gravitational anomalies; magnetic lines and completeness; accidental and non-invertible symmetries of the SM; pions and WZW terms from anomaly matching; neutrino masses, SMEFT, and what is not explained.}
...
\roadchapter{VI.4}{Duality as change of polarization}
Electric--magnetic duality, T-duality, mirror symmetry, and S-duality as changes in which variables, charges, and defects are treated as elementary. Charge lattices, Dirac pairings, line operators, and boundary conditions are the invariant data; compactification turns one duality into another; duality groups act on backgrounds as well as dynamical fields. This chapter ties the 2+1d web to Seiberg--Witten theory, class $\mathcal{S}$, and the holographic dictionaries of Part XI.
\chsections{electric--magnetic duality and charge lattices; T-duality and compactification; mirror symmetry; S-duality and line operators; boundary conditions as polarizations; duality groups acting on backgrounds.}
\chreading{Tachikawa \arxiv{1312.2684}; Gaiotto \arxiv{0904.2715}; Kapustin--Seiberg \arxiv{1401.0740}.}
...
\roadchapter{VIII.4}{Finite density and Fermi surfaces}
Most gapless quantum matter is not a CFT. The Fermi liquid as a fixed point with a codimension-one manifold of gapless modes; patch EFT and the RG around a Fermi surface; Luttinger's theorem as anomaly matching; emergent symmetries acting separately on each patch; non-Fermi liquids from Fermi surfaces coupled to gauge fields; Lifshitz and other nonrelativistic scalings as local QFTs outside the relativistic axiom templates.
\chsections{finite density as a deformation; Fermi liquid fixed points; patch RG; Fermi-surface anomalies and Luttinger; non-Fermi liquids and gauge fields; Lifshitz/nonrelativistic scaling.}
\chreading{Shankar, \emph{Rev.\ Mod.\ Phys.}\ \textbf{66} (1994) 129; Ma--Wang (\texttt{2110.09492}); Wang--Hickey--Ying--Burkov (\texttt{2110.10692}).}

\roadchapter{VIII.5}{Exotica: fractons and UV/IR mixing}
...
\roadchapter{VIII.6}{Rigor: constructive QFT and triviality}
...
\noin \emph{The on-shell program and the S-matrix bootstrap: carve out the space of consistent theories from locality, unitarity, analyticity, and crossing --- in flat space this time.}

\roadchapter{IX.1}{Modern amplitudes}
The computational revolution that replaced Feynman diagrams as the native language of scattering: spinor-helicity variables, on-shell recursion, generalized unitarity, color--kinematics duality and the double copy, positive geometry where it exists. The moral is the same as elsewhere in the book: locality and unitarity leave sharper fingerprints on observables than on Lagrangians.
\chsections{spinor-helicity and little group; BCFW recursion; generalized unitarity; color--kinematics and double copy; amplitudes in EFT; positive geometries and their limits.}
\chreading{Elvang--Huang, \emph{Scattering Amplitudes in Gauge Theory and Gravity}; Bern et al.\ (\texttt{2203.13013}) for color--kinematics and double copy.}

\roadchapter{IX.2}{The S-matrix bootstrap}
The 1960s program with 2020s technology: first, when a flat-space S-matrix is an observable at all (mass gap, asymptotic states, massless IR sectors and inclusive observables); then nonperturbative two-to-two amplitudes constrained by unitarity + analyticity + crossing, explored numerically (primal and dual); pion physics as a target; QFT in AdS as the regulator connecting the S-matrix bootstrap to the conformal bootstrap of Part III.
\chsections{when the S-matrix exists; massless IR and inclusive observables; axioms for amplitudes; numerical primal/dual methods; the pion bootstrap; QFT in AdS; relation to the conformal bootstrap.}
\chreading{Kruczenski--Penedones--van~Rees \arxiv{2203.02421}; Paulos et al.\ (\texttt{1607.06109}).}

\roadchapter{IX.3}{Positivity and the UV--IR connection}
...
\roadchapter{IX.4}{Toward flat-space holography}
The frontier chapter: soft theorems reorganized as symmetries of the S-matrix; memory effects and the IR triangle; celestial and Carrollian proposals for a holographic description of flat space; what is established, what is aspirational, and what the S-matrix bootstrap says either way.
\chsections{soft theorems as Ward identities; memory and asymptotic symmetries; celestial sphere kinematics; Carrollian proposals; status report, written skeptically.}
...
Why regions of a QFT have no density matrices: local algebras, Type III, and the failure of factorization --- stated as physics, not pathology; the split property as the controlled approximation to tensor factorization; Tomita--Takesaki modular theory and what modular flow means (Bisognano--Wichmann: it is a boost); the type classification as a physical classification of how systems entangle.
\chsections{algebras instead of tensor factors; Types I/II/III with physical fingerprints; the split property; modular flow; Bisognano--Wichmann; half-sided modular inclusions (a glimpse).}
...
\subsection*{Methods (Parts III.6, IV.6)}
\begin{itemize}
