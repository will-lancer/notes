...
1.00115965218059(13)$ agrees with the Standard Model prediction
...
\end{equation}
where the Gaussian moments are $(4n-1)!! = (4n)!/\left(2^{2n}(2n)!\right)$, so that
\begin{equation}
...
\arxiv{1802.10441} for a thorough introduction, and Part IV.6 for where it sits
...
stand and at what resolution you look. (Part VIII.6; the $d = 4-\epsilon$
...
\begin{equation}
H = \frac{1}{2}\left(\Pi - \frac{\theta}{2\pi}\right)^2 .
\end{equation}
...
\begin{equation}
E_n = \frac{1}{2}\left(n - \frac{\theta}{2\pi}\right)^2 .
...
Z(\beta, \theta) = \tr e^{-\beta H}
  = \sum_{n \in \ints}
    \exp\left[-\frac{\beta}{2}\left(n - \frac{\theta}{2\pi}\right)^2\right].
...
combination to hold fixed is the physical gap. Writing $T = e^J\left(I +
e^{-2J}\sigma_x + \cdots\right)$ and matching to $T \simeq e^{-aH +
\text{const}}$,
\begin{equation}
\begin{aligned}
H &= -\Gamma \sigma_x,\\
\Gamma &= \frac{e^{-2J}}{a} \text{ held fixed as } a \to 0 .
\end{aligned}
...
continuum descriptions that violate cherished axioms (fractons, Part VIII.5) or
...
exact answer is
\begin{equation*}
\frac{\lambda_+^{N-r}\lambda_-^{r} + \lambda_-^{N-r}\lambda_+^{r}}
{\lambda_+^N + \lambda_-^N}.
\end{equation*}}
\end{exercise}
...
independent,
\begin{equation*}
[\op_1(x), \op_2(y)] = 0 \text{ for } (x-y)^2 > 0,
\end{equation*}
...
boundary in the wrong place. Fractons (Part VIII.5) push on exactly this seam.
\end{nnote}
...
\hint{$\Delta(x-y) = \int \frac{d^3p}{(2\pi)^3 2E_p}\, \left( e^{ip\cdot(x-y)} -
e^{-ip\cdot(x-y)} \right)$ with $p \cdot x = -E t + \mathbf{p} \cdot \mathbf{x}$ in
...
\begin{aligned}
[\phi] &= \frac{d-2}{2},\\
\left[\textstyle\int d^dx\, g_n \phi^n\right] &= 0,\\
[g_n] &= d - n\frac{d-2}{2} .
\end{aligned}
